...
function parseChampionStats(value) {
  try {
    ...
    return {
      ...
      baseHealth: String(parsed.baseHealth || '').trim(),
      baseAttack: String(parsed.baseAttack || '').trim(),
      attackGrowth: String(parsed.attackGrowth || '').trim(),
      healthGrowth: String(parsed.healthGrowth || '').trim(),
      healthMultiplier: String(parsed.healthMultiplier || '').trim(),
      attackMultiplier: String(parsed.attackMultiplier || '').trim(),
      ...
      critRate: String(parsed.critRate || '').trim(),
      critDamage: String(parsed.critDamage || '').trim(),
    };
  } catch {
    return {};
  }
